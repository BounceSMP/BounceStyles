    ...
    public static void handle(EquipStyleC2S msg, Supplier<NetworkEvent.Context> contextSupplier) {
        contextSupplier.get().enqueueWork(() -> {
            ...
            if(msg.style == null || styleData.hasStyleUnlocked(msg.style) || (player.isCreative() && player.hasPermissionLevel(2))) {
                switch (msg.category) {
                    case Head: styleData.setHeadStyle(msg.style); break;
                    case Body: styleData.setBodyStyle(msg.style); break;
                    case Legs: styleData.setLegStyle(msg.style); break;
                    case Feet: styleData.setFeetStyle(msg.style); break;
                }
