...
public class WardrobeStyleWidget extends ClickableWidget {
    ...
    public static class StyleButton extends ButtonWidget {
        ...
        public void renderButton(MatrixStack stack, int mouseX, int mouseY, float partialTick) {
            ...
            RenderSystem.translatef(this.x + (this.width / 2), this.y + this.height, 1050.0F);
            RenderSystem.scalef(1.0f, 1.0f, -1.0f);
            MatrixStack poseStack2 = new MatrixStack();

            float offsetY = 0f;
            switch (this.parentWidget.category) {
                case Head: offsetY = -(float)(.5f); break;
                case Body: offsetY = -(1f); break;
                case Legs:
                case Feet:
                    offsetY = -1.8F; break;
            }
        ...
        public void onPress() {
            if(this.parentWidget.selectedButton == this) {
                BounceStylesNetwork.NETWORK.sendToServer(new EquipStyleC2S(this.parentWidget.category, null));
                this.parentWidget.selectedButton = null;
            }
            else {
                BounceStylesNetwork.NETWORK.sendToServer(new EquipStyleC2S(this.parentWidget.category, this.style));
