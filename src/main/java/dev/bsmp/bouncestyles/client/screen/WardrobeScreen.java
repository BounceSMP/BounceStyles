...
public class WardrobeScreen extends Screen {
    ...
    protected void init() {
        ...
        this.previewWidget = addButton(new WardrobePreviewWidget(0, 0, previewRight, height, client.player));
        this.categoryWidget = addButton(new WardrobeCategoryWidget(this, previewRight, 1, width - previewRight - 48, topBarHeight));
        this.styleWidget = addButton(new WardrobeStyleWidget(previewRight, topBarHeight + 2, width - previewRight, height - topBarHeight));

        int btnSize = topBarHeight;
        this.clearButton = addButton(new ScaledImageButton(new LiteralText("Clear Equipped"), width - topBarHeight, 1, btnSize, btnSize, 98, 0, 24, 24, TEX_WIDGETS, button -> clear()));
        this.toggleArmorButton = addButton(new ScaledImageButton(new LiteralText("Toggle Armor Visibility"),width - (topBarHeight * 2), 1, btnSize, btnSize, 122, 0, 24, 24, TEX_WIDGETS, button -> toggleArmor()));

        this.presetName = addButton(new TextFieldWidget(client.textRenderer, previewRight + topBarHeight + 10, height - topBarHeight - 5, (width - previewRight) / 2, topBarHeight, new LiteralText("Preset Name")));
    ...
    public void render(MatrixStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        if(this.presetsWidget != null)
            this.presetsWidget.render(poseStack, mouseX, mouseY, partialTick);
    }
    ...
    @Override
    public void tick() {
        if(this.presetsWidget != null && this.presetsWidget.needsRefreshing)
            this.presetsWidget.refreshEntries();
    }

    private void clear() {
        BounceStylesNetwork.NETWORK.sendToServer(new EquipStyleC2S(StyleLoader.Category.Head, null));
        BounceStylesNetwork.NETWORK.sendToServer(new EquipStyleC2S(StyleLoader.Category.Body, null));
        BounceStylesNetwork.NETWORK.sendToServer(new EquipStyleC2S(StyleLoader.Category.Legs, null));
        BounceStylesNetwork.NETWORK.sendToServer(new EquipStyleC2S(StyleLoader.Category.Feet, null));
//        this.styleWidget.selectedButton = null;
    }

    private void toggleArmor() {
        BounceStylesNetwork.NETWORK.sendToServer(new ToggleArmorVisibilityC2S());
    }
    ...
    public void setSelectedCategory(StyleLoader.Category category) {
        ...
        if (category == StyleLoader.Category.Preset) {
            ...
            this.presetsWidget = addChild(new WardrobePresetsWidget(client, this, previewRight, topBarHeight, width - previewRight, height - topBarHeight, 30, topBarHeight));
        }
        else {
            this.children.remove(this.presetsWidget);
