...
public class StyleLayerRenderer<T extends LivingEntity, M extends BipedEntityModel<T>, A extends BipedEntityModel<T>> extends FeatureRenderer<T, M> implements IGeoRenderer<Style> {
    ...
    public StyleLayerRenderer(FeatureRendererContext<T, M> renderer) {
        super(renderer);
        this.modelProvider = new StyleModel();
    }

    @Override
    public void render(MatrixStack poseStack, VertexConsumerProvider buffer, int packedLight, T entity, float limbSwing, float limbSwingAmount, float partialTick, float animationProgress, float headYaw, float headPitch) {
    ...
    public void renderStyle(MatrixStack poseStack, Style style, PlayerEntity player, StyleLoader.Category category, VertexConsumerProvider buffer, float limbSwing, float limbSwingAmount, float partialTick, int packedLight) {
        if(GeckoLibCache.getInstance().getGeoModels().containsKey(style.modelID)) {
            ...
            this.modelProvider.setCustomAnimations(style, getUniqueID(style), new AnimationEvent<>(style, limbSwing, limbSwingAmount, partialTick, false, Collections.singletonList(player)));
    ...
    private void fit(MatrixStack poseStack, StyleLoader.Category category, boolean gui) {
        ...
        PlayerEntityModel<PlayerEntity> playerModel = (PlayerEntityModel<PlayerEntity>) getContextModel();
        ...
        switch (category) {
            ...
            case Body:
                ...
                if(!gui) {
                    ...
                    RenderUtils.moveBackFromPivot(bodyGeoBone, poseStack);
                }
            ...
            case Legs:
                ...
                if(!gui) {
                    ...
                    RenderUtils.moveBackFromPivot(rightLegGeoBone, poseStack);
                    RenderUtils.moveBackFromPivot(leftLegGeoBone, poseStack);
                }
            ...
            case Feet:
                ...
                if(!gui) {
                    ...
                    RenderUtils.moveBackFromPivot(rightBootGeoBone, poseStack);
                    RenderUtils.moveBackFromPivot(leftBootGeoBone, poseStack);
                }
